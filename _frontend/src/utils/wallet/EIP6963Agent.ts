...
import {
  HEDERA_LOGO,
  networkToChainId,
  WalletClient_Ethereum,
} from "@/utils/wallet/client/WalletClient_Ethereum";
import {
...
export class EIP6963Agent {
  ...
  public async requestSession(
    ...
    if (d !== null) {
      ...
      try {
        ...
        result = new WalletSession_EIP6963(
          d,
          d.info.name,
          d.info.icon,
          usableAccountIds,
  ...
  public async checkAccessWithRDN(
    providerRDN: string,
  ): Promise<WalletSession | null> {
    const d =
      this.providers.value.find(
        (d: EIP6963ProviderDetail) => d.info.rdns === providerRDN,
      ) ?? null;
  ...
  // EIP6963Agent.ts snippet
  private readonly handleAnnounceProvider = (
    event: EIP6963AnnounceProviderEvent,
  ): void => {
    const newUUID = event.detail.info.uuid;
    // check if we already have it
    const match = this.providers.value.find((p) => p.info.uuid === newUUID);
    if (!match) {
      // This makes the wallet appear in your "All Wallets" list
  ...
  private findProviderDetails(uuid: string): EIP6963ProviderDetail | null {
    const result =
      this.providers.value.find(
        (d: EIP6963ProviderDetail) => d.info.uuid === uuid,
      ) ?? null;
    if (result === null) {
...
class WalletSession_EIP6963 extends WalletSession {
  ...
  public makeClient(accountId: string): Promise<WalletClient | null> {
    const result = new WalletClient_Ethereum(
      accountId,
...
const CHAIN_PARAM_MAINNET: AddEthereumChainParameter = {
  chainId: "0x127",
  blockExplorerUrls: ["https://hashscan.io/mainnet/home"],
...
const CHAIN_PARAM_TESTNET: AddEthereumChainParameter = {
  chainId: "0x128",
  blockExplorerUrls: ["https://hashscan.io/testnet/home"],
...
const CHAIN_PARAM_PREVIEWNET: AddEthereumChainParameter = {
  chainId: "0x129",
  blockExplorerUrls: ["https://hashscan.io/previewnet/home"],
  chainName: "Hedera Previewnet",
  iconUrls: [HEDERA_LOGO],
  nativeCurrency: NATIVE_CURRENCY,
  rpcUrls: ["https://previewnet.hashio.io/api"],
};
const CHAIN_PARAM_MPCQNET: AddEthereumChainParameter = {
  chainId: "0x12A",
  blockExplorerUrls: ["http://localhost:5173/mpcqnet/home"],
  chainName: "Hedera MPCQNET",
  iconUrls: [HEDERA_LOGO],
  nativeCurrency: NATIVE_CURRENCY,
