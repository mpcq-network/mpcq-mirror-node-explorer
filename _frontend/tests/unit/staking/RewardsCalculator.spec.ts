...
describe("RewardsCalculator.vue", () => {
    ...
    it("should display an empty Rewards Estimator", async () => {
        ...
        expect(options[0].element.text).toBe('0 - Hosted by MPCQ | East Coast, USA - Rewarding (staked for reward is 16.7% of max)')
        expect(options[1].element.text).toBe('1 - Hosted by MPCQ | East Coast, USA - Rewarding (staked for reward is 23.3% of max)')
        expect(options[2].element.text).toBe('2 - Hosted by MPCQ | Central, USA - Rewarding (staked for reward is 23.3% of max)')
